...
#Magda Arnal
#14/02/2023
#Run the final model with the GB algorithm
"""
...
parser.add_option('-e', '--Nestimators',
                  help='HyperParameter: number of estimators.',
                  metavar=50)
parser.add_option('-r', '--LearningRate',
                  help='HyperParameter: learning rate.',
                  metavar=0.001)
parser.add_option('-a', '--SubSample',
                  help='HyperParameter: subsample.',
                  metavar=0.5)
parser.add_option('-d', '--MaxDepth',
                  help='HyperParameter: max depth.',
                  metavar=12)
parser.add_option('-u', '--Loss',
                  help='HyperParameter: loss.',
                  metavar='deviance')
...
H_lr = float(option_dict['LearningRate'])
H_sb = float(option_dict['SubSample'])
...
if sampl_strategy in ['random','ENN'] :
    ...
    f1_output=outDir+'Fold'+str(manualFold)+'_GB_TrainingIDs.txt'
...
modTree = GradientBoostingClassifier(n_estimators=H_ne,
                                     learning_rate=H_lr,
                                     subsample=H_sb,
                                     max_depth=H_md,
                                     loss=H_lo,
...
f2_output=outDir+'Fold'+str(manualFold)+'_GB_Predictors.txt'
...
f3_output=outDir+'Fold'+str(manualFold)+'_GB_Samples.txt'
x_telab.to_csv(f3_output, index=None, sep='\t')

#save the model
joblib.dump(modTree, outDir + 'Trained_GB_'+'Fold'+str(manualFold)+'.joblib', compress=3)

####################################################################################################
####################################################################################################
#Confirm that the metrics are the same after loading the model
loaded_modTree=joblib.load(outDir + 'Trained_GB_'+'Fold'+str(manualFold)+'.joblib')
...
c=(H_ne,H_lr, H_sb, H_md, H_lo, balance, sampl_strategy)
Hiper='|'.join(str(p) for p in c)

with open(GlobalResults, 'a+') as f:
    f.write(PatCond +'\t'+'GB'+'\t'+ inMtrxName +
    '\t' + str(PredNum) +
    '\t' + str(CntrlNumber) +
    '\t' + str(CaseNumber) +
    '\t' + 'Hiper:' + Hiper +
