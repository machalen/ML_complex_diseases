...
#Magda Arnal
#14/02/2023
#Run the final model with the ET algorithm
"""
...
parser.add_option('-f', '--fold',
                  ...
                  metavar='4')
parser.add_option('-e', '--Nestimators',
                  help='HyperParameter: n_estimators.',
                  metavar=50)
parser.add_option('-a', '--minSamplesSplit',
                  help='HyperParameter: min_samples_split.',
                  metavar=2)
parser.add_option('-i', '--minSamplesLeaf',
                  help='HyperParameter: min_samples_leaf.',
                  metavar=5)
parser.add_option('-d', '--MaxDepth',
                  help='HyperParameter: max depth.',
                  metavar=None)
...
H_ne = int(option_dict['Nestimators'])
H_ss = int(option_dict['minSamplesSplit'])
H_sl = int(option_dict['minSamplesLeaf'])
H_md = option_dict['MaxDepth']
if H_md != 'None':
    H_md=int(H_md)
else:
    H_md=None
...
################################################################################
##############################Make nested CV####################################
np.random.seed(0)#set random seed to allow reproducibility
...
if sampl_strategy in ['random','ENN'] :
    ...
    f1_output=outDir+'Fold'+str(manualFold)+'_RF_TrainingIDs.txt'
...
#Define the model with the parameters defined in the input
modTree = RandomForestClassifier(n_estimators=H_ne,
                               min_samples_split=H_ss,
                               min_samples_leaf= H_sl,
                               max_depth=H_md,
                               random_state=0)
modTree.fit(x_train, y_train)

#Build the dataframe with the feature importances
...
f2_output=outDir+'Fold'+str(manualFold)+'_RF_Predictors.txt'
...
f3_output=outDir+'Fold'+str(manualFold)+'_RF_Samples.txt'
x_telab.to_csv(f3_output, index=None, sep='\t')

#save the model
joblib.dump(modTree, outDir + 'Trained_RF_'+'Fold'+str(manualFold)+'.joblib', compress=3)

####################################################################################
####################################################################################
#Confirm that the metrics are the same after loading the model
loaded_modTree=joblib.load(outDir + 'Trained_RF_'+'Fold'+str(manualFold)+'.joblib')
yhat=loaded_modTree.predict(x_test)
...
c=(H_ne,H_ss, H_sl,H_md,balance,sampl_strategy)
Hiper='|'.join(str(p) for p in c)

with open(GlobalResults, 'a+') as f:
    f.write(PatCond +'\t'+'RF'+'\t'+ inMtrxName +
    '\t' + str(PredNum) +
    '\t' + str(CntrlNumber) +
    '\t' + str(CaseNumber) +
    '\t' + 'Hiper:' + Hiper +
