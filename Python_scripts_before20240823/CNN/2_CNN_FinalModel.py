...
from CNN_classes import CNN_WithCV
from CNN_classes import PlotPerformance
from CNN_classes import CNNclassWithSigmoid
...
def trainTheModel(numEp,learningRate, selfdr,nUnits,nLayers,sampl_strategy,balance):
    ...
    if sampl_strategy == 'ENN' :
        ...
        #sampl = {1: minority_class_size, 0:0}#The dictionary doesn't work!
    ...
    for fold, (train_index, dev_index) in enumerate(strat.split(X_train_dev, y_train_dev)):
        ...
        if sampl_strategy == 'random' :
            ...
            #Apply ENN
            X_renum, y_resampled = enn.fit_resample(x_num, Y_train)
            subsampled_indices = enn.sample_indices_
            X_resampled=X_train.iloc[subsampled_indices]
            # unique, counts = np.unique(y_resampled, return_counts=True)
            # print('Counts after undersampling:',np.asarray((unique, counts)).T)
            x_trtab, y_train = rus.fit_resample(X_resampled, y_resampled)#Set the proportion of the balancing
            # unique, counts = np.unique(Y_train, return_counts=True)
            # print('Counts after undersampling:',np.asarray((unique, counts)).T)
        elif sampl_strategy == 'SMOTE_random' :
            x_num=X_train.iloc[:,labels.shape[1]:len(X_train.columns)]
            ...
            x_trtab, y_train = rus.fit_resample(X_oversampled, y_oversampled)#Set the proportion of the balancing
            # unique, counts = np.unique(Y_train, return_counts=True)
            # print('Counts after undersampling:',np.asarray((unique, counts)).T)
        elif sampl_strategy == 'SMOTE_ENN' :
            #Convert the X_trtab to numeric for enn method:
        ...
        #Check that the cases and controls are properly assigned after train_test split
        con2=x_devtab.iloc[:,1]
        yte=np.where(con2==PatCond, 1, con2)
        yte=np.where(yte=='control', 0, yte)
        yte=yte.astype('int')
        print(np.array_equal(y_dev, yte))#TRUE!
        #Save the labels
...
#Calculate the evaluation metrics
yHat_pre=(yHat>0).float()#Translate continuous to 0/1
m_acc0=torch.mean((yHat_pre[y_test == 0] == y_test[y_test == 0]).float()).item()
m_acc1=torch.mean((yHat_pre[y_test == 1] == y_test[y_test == 1]).float()).item()
m_acc = torch.mean((yHat_pre == y_test).float()).item()
...
#I have to change the output of the CNN to produce a probability value with the sigmoid function
net_a.cpu()
net_b = CNNclassWithSigmoid(net_a.dr, nUnits, net_a.nLayers, PredNum)
net_b.load_state_dict(net_a.state_dict())
net_b.to(device)

#Confirm that the evaluation metrics are the same as in the previous model
net_b.eval()
torch.manual_seed(1)
yHat = net_b(x_test)
#Bring back
yHat=yHat.cpu()
yHat_pre=(yHat>0.5).float()#Translate continuous to 0/1
...
#Check the order of the elements in the new matrix
attr_lig_np[0,0,0] == attr_lig_mtrx[0,0]
attr_lig_np[0,0,1] == attr_lig_mtrx[0,1]
...
#Check the order of the elements in the new matrix
attr_dl_np[0,0,0] == attr_dl_mtrx[0,0]
attr_dl_np[0,0,1] == attr_dl_mtrx[0,1]
#Save attributes
...
#Check the order of the elements in the new matrix
attr_sl_np[0,0,0] == attr_sl_mtrx[0,0]
attr_sl_np[0,0,1] == attr_sl_mtrx[0,1]
#Save attributes
...
#Check the order of the elements in the new matrix
attr_gbp_np[0,0,0] == attr_gbp_mtrx[0,0]
attr_gbp_np[0,0,1] == attr_gbp_mtrx[0,1]
#Save attributes
