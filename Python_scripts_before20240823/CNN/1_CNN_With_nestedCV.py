...
print ('Train/Dev set:', X_train_dev_np.shape,  y_train_dev.shape)
...
print ('Test set:', X_test_np.shape,  y_test.shape)
...
def trainTheModel(numEp,learningRate, selfdr,nUnits,nLayers,Sampling, Balance):
    ...
    for fold, (train_index, dev_index) in enumerate(strat.split(X_train_dev_np, y_train_dev)):
        ...
        if Sampling == 'random' :
            ...
            # print('Counts after undersampling:',np.asarray((unique, counts)).T)
            X_train_np, Y_train = rus.fit_resample(X_resampled, y_resampled)#Set the proportion of the balancing
            # unique, counts = np.unique(Y_train, return_counts=True)
            # print('Counts after undersampling:',np.asarray((unique, counts)).T)
        elif Sampling == 'SMOTE_random' :
            ...
            X_train_np, Y_train = rus.fit_resample(X_oversampled, y_oversampled)#Set the proportion of the balancing
            # unique, counts = np.unique(Y_train, return_counts=True)
            # print('Counts after undersampling:',np.asarray((unique, counts)).T)
        elif Sampling == 'SMOTE_ENN' :
            X_oversampled, y_oversampled = sm.fit_resample(x_train_np, y_train)
...
# SamplingV=['random','ENN', 'SMOTE_random','SMOTE_ENN']#Sampling methods
