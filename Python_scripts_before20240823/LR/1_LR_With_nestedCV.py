...
#Magda Arnal
#07/07/2023
#Run the LR algorithm with nested CV 
"""
### import libraries
...
# grid = dict()
# grid['solver'] =  ['newton-cg', 'liblinear', 'sag', 'saga']
# grid['creg'] = [0.0001, 0.001, 0.01, 1, 10, 100]
# grid['balance'] = [50, 60, 70]
...
grid['creg'] = [0.01]
grid['balance'] = [50]
grid['sampl_strategy'] = ['random','SMOTE_ENN']
...
for s in range(len(grid[keys_list[0]])):
    so=grid[keys_list[0]][s]
    for r in range(len(grid[keys_list[1]])):
        cr=grid[keys_list[1]][r]
        for b in range(len(grid[keys_list[2]])):
            ba=grid[keys_list[2]][b]
            for m in range(len(grid[keys_list[3]])):
                sa=grid[keys_list[3]][m]
                c=(so,cr,ba,sa)
                combination = {'solver':so,
                              'Creg':cr,
                              'Balance':ba,
                              'Sampling':sa}
...
f_output=outDir+PatCond+'_fold'+str(manualFold)+'_LR_EvMetrics_CV.txt'
