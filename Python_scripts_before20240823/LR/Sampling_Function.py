...
def Cross_Val_Groups(model, X, y, combination, n_splits = 10):
    ...
    for train_index, val_index in strat.split(X, y):
        ...
        modLR = model.set_params(solver=combination['solver'],
                                   C=combination['Creg'],
                                   random_state=0)
        modLR.fit(X_train, Y_train)

        #Prediction and evaluation on the training set
        pred_train = modLR.predict(X_train)
        ...
        mpred = modLR.predict_proba(X_train)
        pred=mpred[:,1]
        ...
        #Calculate the roc curve
        mpred = modLR.predict_proba(X_val)
        pred=mpred[:,1]
