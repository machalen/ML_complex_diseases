...
if sampl_strategy in ['random','ENN'] :
    ...
    f1_output=outDir+'Fold'+str(manualFold)+'_ET_TrainingIDs.txt'
...
modTree = ExtraTreesClassifier(n_estimators=H_ne,
...
f2_output=outDir+'Fold'+str(manualFold)+'_ET_Predictors.txt'
...
f3_output=outDir+'Fold'+str(manualFold)+'_ET_Samples.txt'
x_telab.to_csv(f3_output, index=None, sep='\t')

#save the model
joblib.dump(modTree, outDir + 'Trained_ET_'+'Fold'+str(manualFold)+'.joblib', compress=3)

#####################################################################################
#####################################################################################
#Confirm that the metrics are the same after loading the model
loaded_modTree=joblib.load(outDir + 'Trained_ET_'+'Fold'+str(manualFold)+'.joblib')
